export class TodosContent {
    ...
    constructor(selector, checkCallback, removeCallback) {
    ...
    render(todos) {
        ...
        todos.forEach(todo => {
            this.$element.insertAdjacentHTML('beforeend', `
                <div class="todo_list-item${todo.done ? ' completed' : ''}" draggable="true">
    ...
    check(event) {
        const id = Number(event.path.find(element => element.id).id.replace('check-todo-', ''))
        this.checkCallback(id)
    }

    remove(event) {
        const id = Number(event.path.find(element => element.id).id.replace('remove-todo-', ''))
